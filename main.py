...
def converter_pdf():
    ...
    if extensao == '.txt':
        ...
        docx_to_pdf(os.path.join(app.config['UPLOAD_FOLDER'], filename), pdf_path)
    elif extensao == '.xlsx':
        xlsx_to_pdf(os.path.join(app.config['UPLOAD_FOLDER'], filename), pdf_path)

    if os.path.isfile(pdf_path):
        # Retorna o arquivo PDF como um download
...
def docx_to_pdf(docx_path, pdf_path):
    ...
    try:
        ...
        print(f"Erro ao converter arquivo DOCX para PDF: {e}")
